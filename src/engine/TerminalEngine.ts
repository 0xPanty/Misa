...
// 配置从 window.__MISA_CONFIG__ 获取
const getConfig = () => (window as any).__MISA_CONFIG__ || {
  typeSpeed: 30,
  mouthInterval: 120,
  beepFrequency: 800,
  beepDuration: 0.03,
...
export class TerminalEngine {
  ...
  private tick(timestamp: number) {
    const cfg = getConfig();
    const { typeSpeed, mouthInterval } = cfg;
  ...
  private playBeep() {
    ...
    const cfg = getConfig();
    const { beepFrequency, beepDuration, soundVolume, beepVolumeEnd } = cfg;
